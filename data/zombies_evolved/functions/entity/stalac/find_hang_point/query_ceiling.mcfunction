...
execute store result storage zombies_evolved:data func.find_hang_point.out.height int 1 run scoreboard players get #_height zev._
...
data modify storage zombies_evolved:data func.find_hang_point.out.valid set value 0b
execute unless score #_height zev._ matches -1 positioned as @s align xyz run tp @s ~0.5 ~ ~0.5
execute store success storage zombies_evolved:data func.find_hang_point.out.valid byte 1 unless score #_height zev._ matches -1 positioned as @s if block ~ ~ ~ #zombies_evolved:stalac_hangable align xyz unless entity @e[dy=-1,type=#zombies_evolved:can_specialize,tag=zev.stalac.hanging,limit=1] run data modify storage zombies_evolved:data func.find_hang_point.out.hang_point set from entity @s Pos
