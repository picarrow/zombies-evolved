...
execute positioned as @s run function zombies_evolved:stalac/crumble/spawn_helpers_4

# if dy and the originating block is valid, succeed otherwise kill
scoreboard players set #_successful zev._ 0
execute store success score #_successful zev._ unless score #_dy zev._ matches 3 positioned as @s if block ~ ~ ~ #zombies_evolved:cubic run function zombies_evolved:stalac/crumble/succeed
